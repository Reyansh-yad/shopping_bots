...
const navAnimation = () => {
  ...
  // Add scroll event listener (only if a <nav> exists)
...
const page2Animation = () => {
  if (!document.querySelector("#slide1")) return;
  gsap.to("#slide1 h2", {
  ...
  if (swiperEl) {
    swiperEl.addEventListener("mouseenter", function (dets) {
      gsap.to("#cursor2", {
        scale: 1,
      });
    });
    swiperEl.addEventListener("mouseleave", function (dets) {
      gsap.to("#cursor2", {
        scale: 0,
      });
    });
